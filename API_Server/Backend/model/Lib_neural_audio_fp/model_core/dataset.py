...
class Dataset:
    ...
    def __set_augmentation_fps(self):
        ...
        #import os
        #print(os.getcwd())
        #print(f"self.source_root_dir: {os.path.exists(self.source_root_dir)}\n\t{self.source_root_dir}")
    ...
    def get_train_ds(self, reduce_items_p=0):
        ...
        if self.datasel_train == '10k_icassp':
            ...
            glob.glob(self.source_root_dir + _prefix + '**/*.wav',
                      recursive=True))
        elif  self.datasel_train=="":
            assert os.path.exists(os.path.join(self.source_root_dir, ""))
            self.tr_source_fps = sorted(
        ...
        # print(f"From dataset:\n\tself.tr_source_fps = {len(self.tr_source_fps)}\n\tpath = {self.source_root_dir + _prefix + '**/*.wav'}")
        ds = genUnbalSequence(
