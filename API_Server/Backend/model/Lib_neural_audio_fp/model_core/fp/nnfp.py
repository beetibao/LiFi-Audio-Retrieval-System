...
def get_fingerprinter(cfg, trainable=False):
    ...
    """
    len_src = int(cfg['MODEL']['DUR']*cfg['MODEL']['FS'])
    n_segs = math.ceil((len_src-cfg['MODEL']['STFT_WIN'])/cfg['MODEL']['STFT_HOP'])
    n_segs = int(n_segs)

    input_shape = (int(cfg['MODEL']['N_MELS']), n_segs, 1)
    emb_sz = cfg['MODEL']['EMB_SZ']
    norm = cfg['MODEL']['BN']
    fc_unit_dim = [n_segs, 1]
